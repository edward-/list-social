...
import { UpdateProps } from '@/lib/interfaces';
import { Status, Item, TodoList } from '@/lib/definitions';
import { defaultTodoList } from '@/lib/data';

import type { Schema } from '@/../amplify/data/resource';
import { generateClient } from 'aws-amplify/data';
import { transformTodo } from '@/lib/transform';
import { getStatus } from '@/lib/utils';

const client = generateClient<Schema>();

export default function UpdateList({ intl, todoListId, closeDetails }: UpdateProps) {
  const [todoList, setTodoList] = useState(defaultTodoList);

  const transformTodoToTodoList = useCallback((todo: Schema['Todo']['type']) => transformTodo(todo), []);

  useEffect(() => {
    const getList = async () => {
      const { data: todo } = await client.models.Todo.get({ id: todoListId });
      if (todo) {
        setTodoList(transformTodoToTodoList(todo));
      }
    };
    getList();
  }, []);
  ...
  const deleteTodoList = useCallback((todoListId: string) => {
    const deleteIt = async () => {
      const { errors } = await client.models.Todo.delete({ id: todoListId });
    };
    deleteIt();
  }, []);

  const deleteList = (e: React.ChangeEvent<any>) => {
    e.preventDefault();
  ...
  const updateTodoList = useCallback((todoListId: string, todoList: TodoList) => {
  ...
  const changeStatus = useCallback(
    (itemToUpdate: Item) => {
      switch (itemToUpdate.status) {
        case Status.HOLDON:
          itemToUpdate.status = Status.INPROGRESS;
          break;
        case Status.INPROGRESS:
          itemToUpdate.status = Status.DONE;
          break;
        case Status.DONE:
          itemToUpdate.status = Status.CANCELLED;
          break;
        case Status.CANCELLED:
          itemToUpdate.status = Status.HOLDON;
          break;
      }
      const updatedItems = todoList.items.map((item) => (item.order === itemToUpdate.order ? itemToUpdate : item));
      ...
      <div className='relative p-4 w-full max-w-2xl max-h-full'>
        <div className='relative bg-white rounded-lg shadow dark:bg-gray-700'>
              ...
              {todoList.items.map((item) => (
                <div key={item.order} className='relative max-w-sm p-1'>
                  ...
                  <div
                    ...
                    className='absolute inset-y-0 end-0 p-2.5 flex items-center ps-3'
                  >
                    <img
          ...
          <div className='flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600'>
            ...
            >
              {intl ? intl.formatMessage({ id: 'modal-update-list.update' }) : '[Update]'}
            </button>
            <button
              type='button'
              onClick={deleteList}
            ...
            >
              {intl ? intl.formatMessage({ id: 'modal-update-list.delete' }) : '[Delete]'}
            </button>
            <button
              type='button'
              onClick={closeDetails}
            ...
            >
              {intl ? intl.formatMessage({ id: 'modal-update-list.close' }) : '[Close]'}
            </button>
          </div>
        </div>
      </div>
